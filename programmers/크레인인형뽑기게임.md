...
baguni라는 배열을 스택의 역할을 하도록 만들어, for 문으로 1번 열부터 돌며 moves 배열의 움직임을 체크하였다. 그 과정에서 baguni의 최상단의 인형과 현재 열의 꼭대기 인형이 같다면 지워주고, 점수를 올려주는 방식으로 처리하였다. 아니라면 바구니에 담아주고, 다음 moves의 배열의 움직임을 다시 체크하는 식이었다.